...
- **物品管理**：支持物品创建、查看、编辑、删除、搜索、筛选、排序和分页；物品可记录数量单位和规格；手机端物品详情使用底部面板，筛选默认紧凑展示。
...
- **AI 拍照识别**：手机拍照或上传图片，AI 自动识别物品并生成录入候选，可辅助预测数量单位和规格。
- **AI 订单解析**：粘贴订单截图，AI 批量提取商品信息并生成候选，可从订单标题中提取数量单位和规格。
...
5. 新建或编辑物品时填写一次“数量单位”和“规格”，确认列表和详情会展示例如 `1 桶`、`3L`。
6. 在物品详情上传或拍摄一张图片，确认图片可以显示。
7. 在 `/settings` 创建一次备份，确认备份列表出现新记录。
8. 如使用纯 HTTP 局域网访问，确认 `.env` 中 `AUTH_COOKIE_SECURE="false"`，否则登录 Cookie 可能不会回传。
...
- `home-storage-uploads:/app/uploads`：物品图片上传目录，保存处理后的图片和缩略图。
- `home-storage-backups:/app/backups`：数据库备份文件及对应图片快照。
...
- **创建备份**：复制当前 SQLite 数据库，并为现有图片上传目录创建同名 `.uploads` 快照。
- **查看备份**：按时间查看已有备份文件，并显示图片快照数量和体积。
- **恢复备份**：恢复前会要求确认，并先创建当前数据库和图片目录的保护性备份。
- **删除备份**：只允许删除合法备份文件名，并同步清理对应图片快照目录。
