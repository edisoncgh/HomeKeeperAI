...
## Current Slice
M2.1 分类与位置基础管理代码实现和自动验证已完成；等待用户人工确认 `/categories` 与 `/locations` 的移动端和 PC 端 UI 后，再进入 M2.2 物品 API。

## Last Known Good State
M2.1 新增分类/位置 API、管理页面、共用校验、共用 API 辅助和导航入口。`npm run test`、`npm run typecheck`、`npm run lint`、`npm run build`、`npm run db:check` 均通过。临时 SQLite + dev server API 验证通过。
...
- 自动化无阻塞。
- 页面跨端实际观感尚需用户运行 `npm run dev` 后人工确认。
